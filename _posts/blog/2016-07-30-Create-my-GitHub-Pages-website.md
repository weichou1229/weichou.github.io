...
之後新增部落格文章時,就放在`blog`資料夾內.  
　當然也可以不建blog資料夾,Jekyll還有提供其他設定方式,以我的例子,是用`permalink: /:path`的全域設定,
用途為Jekyll給定靜態資源連結位址的格式,其格式就是用permalink定義.  
　所以在根目錄建立內容只有一行的`_config.yml`檔  
