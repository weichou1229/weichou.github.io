---
author: WeiChou
pubDatetime: 2024-06-01T00:00:00Z
modDatetime: 2024-06-01T00:00:00Z
title: Rust | Curated List of Game development
slug: rust-curated-list-of-game-dev
featured: false
draft: true
# ogImage: https://res.cloudinary.com/noezectz/v1663745737/astro-paper/astropaper-x-forestry-og_kqfwp0.png
tags:
  - Rust
description:
  A curated list of game development(for junior engineer) with Rust and Bevy.
---

A curated list of game development(for junior engineer) with Rust and Bevy.

## Basic of Rust

- [Installation and Hello World example](https://doc.rust-lang.org/book/ch01-00-getting-started.html)
- [Common Syntax](https://doc.rust-lang.org/book/ch03-00-common-programming-concepts.html)
- [Naming Conventions](https://github.com/rust-lang/rfcs/blob/master/text/0430-finalizing-naming-conventions.md)

## Guide

- [Useful Notes](https://taintedcoders.com/)
- [Tutorial from ZymartuGames's youtube chaneel](https://youtu.be/B6ZFuYYZCSY?si=4oSMpDq3t2LuNgFu)

## Free material to Learn the Game Development Concept

- [Harvard CS50G](https://cs50.harvard.edu/games/2018/)
